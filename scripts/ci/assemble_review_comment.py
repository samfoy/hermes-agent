...
def _render_item(item: ReviewItem) -> str:
    ...
    if item.detail:
        parts += ["", item.detail]
    # Show both the job-emitted link (e.g. report URL) and the auto-attached
    # job log link. They're different things — a job may have a report artifact
    # AND its own logs page.
    links: list[str] = []
    if item.link:
...
def _render_info_details(items: list[ReviewItem]) -> str:
    ...
    blocks = [_render_item(i) for i in items]
    detail_md = "\n\n---\n\n".join(blocks)
    return f"<details>\n<summary>more info from completed jobs</summary>\n\n{detail_md}\n\n</details>"
...
def render_comment(items: list[ReviewItem], pending_jobs: list[str] | None = None) -> str:
    ...
    pending_text = f"\n<sub>still waiting for {len(pending)} ci job{'s' if len(pending) != 1 else ''}" if len(pending) else ""
    header = f"{MARKER}\n# ૮ >ﻌ< ა ci review {pending_text}\n\n"

    if not items and not pending:
        return f"{header}looks good to me!"
